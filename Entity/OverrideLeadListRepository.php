...
class OverrideLeadListRepository extends LeadListRepository
{
    ...
    foreach ($lists as $l) {
      ...
      if ($dynamic && count($filters)) {
        ...
        if ($newOnly || !$nonMembersOnly) { // !$nonMembersOnly is mainly used for tests as we just want a live count
          ...
          $this->addExtendedFieldJoins($filters, $extendedFieldList, $q);
  ...
  public function getListFilterExpr($filters, &$parameters, QueryBuilder $q, $isNot = false, $leadId = null, $object = 'lead', $listId = null)
  {
    ...
    foreach ($filters as $k => $details) {
      $isExtendedField = in_array($details['field'], array_keys($extendedFieldList)) ? TRUE : FALSE;
  ...
  public function getExtendedFieldList($secure = true)
  {
    ...
    foreach($results as $field){
      ...
      $secure = strpos($field->getObject(), 'Secure') !== FALSE ? "_secure" : "";
      $tableName = 'lead_fields_leads_' . $dataType . $secure . '_xref';
